...
**Goal:** First PR of the **ChatView sub-stack** (chat.a → chat.b → chat.c). Port the uclaw `chat/` cluster's Layer-0 leaf components (selectors, popovers, dialogs, banners/chips) + the tool-rendering pair (`ChatToolBlock`, `ChatToolActivityIndicator`) verbatim. This is the foundation layer everything above (ChatMessages/ChatInput in chat.b, ChatView in chat.c) builds on. Does NOT replace the `chat-view-stub` (that lands in chat.c when the ChatView root is ported).
