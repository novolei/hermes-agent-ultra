 ...
 *
 * 从 Proma 迁移，适配 Tauri 环境。
 *
 * Ported verbatim from uclaw hooks/useCloseTab.ts (Plan FB.c Wave A1).
 * Retargets: @/atoms/* → @/features/chat-agent/atoms/*,
...
import { conversationPromptIdAtom } from '@/features/chat-agent/atoms/system-prompt-atoms'
...
export function useCloseTab(): UseCloseTabReturn {
  ...
  const executeClose = React.useCallback((tabId: string) => {
    ...
    if (tab?.type === 'agent') {
      invoke<boolean>('stop_agent_session', { conversation_id: tab.sessionId }).catch((err) => {
