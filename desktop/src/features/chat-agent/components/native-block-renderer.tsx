...
/**
 * ChatToolBlock — Minimal tool block renderer for NativeBlockRenderer.
 * Renders a tool_use with optional tool_result.
 */
function ChatToolBlock({
  ...
  return (
    <div
      className="rounded border border-border/40 bg-muted/20 px-3 py-2 text-sm"
    ...
    >
      ...
      {result !== undefined && (
        <div className="mt-1 text-foreground/70" data-testid="tool-result">
          {result}
        </div>
      )}
    </div>
  ...
  return (
    <div className={className} data-native-blocks="true">
      {blocks.map((b, idx) => {
        if (b.type === 'text') {
          return (
            <MessageResponse key={`b-${idx}-text`} sessionId={conversationId ?? null}>
              {b.text}
            </MessageResponse>
          )
        }
