 ...
 * - Enter 发送、Escape 取消
 *
 * Ported from uclaw ui/src/components/chat/InlineEditForm.tsx (chat.b Wave B1, verbatim)
 */

import * as React from 'react'
import { Paperclip, SendHorizontal, X } from 'lucide-react'
import { MessageAction } from '@/features/chat-agent/components/ai-elements/message'
import { AttachmentPreviewItem } from './attachment-preview-item'
import { cn } from '@/shared/lib/cn'
import type { ChatMessage, FileAttachment } from '@/features/chat-agent/lib/chat-types'
import { fileToBase64 } from '@/features/chat-agent/lib/file-utils'
import { readAttachment, openFileDialog } from '@/features/chat-agent/lib/tauri-bridge-stub'
...
export function InlineEditForm({ message, onSubmit, onCancel }: InlineEditFormProps): React.ReactElement {
  ...
  React.useEffect(() => {
    ...
    Promise.all(
      imageAttachments.map(async (att) => {
        try {
          const base64 = await readAttachment(att.localPath)
  ...
  const handleSelectAttachments = React.useCallback(async (): Promise<void> => {
    try {
      ...
      addPendingAttachments(result.files.map((file: { filename: string; mediaType: string; size: number; data: string }) => ({
