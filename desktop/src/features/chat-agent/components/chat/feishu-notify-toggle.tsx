...
import { feishuConnectedAtom, sessionFeishuNotifyModeAtom, feishuDefaultNotifyModeAtom } from '@/features/chat-agent/atoms/feishu-atoms'
import { cn } from '@/shared/lib/cn'
import type { FeishuNotifyMode } from '@/features/chat-agent/lib/chat-types'
import { setFeishuSessionNotify } from '@/features/chat-agent/lib/tauri-bridge-stub'
...
export function FeishuNotifyToggle({ sessionId }: FeishuNotifyToggleProps): React.ReactElement | null {
  ...
  const handleClick = React.useCallback(() => {
    ...
    // 同步到主进程
    setFeishuSessionNotify(sessionId, nextMode).catch(console.error)
