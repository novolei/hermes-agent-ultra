 ...
 *
 * Ported verbatim from uclaw ui/src/components/chat/ChatView.tsx
 * Retargets: @/components/chat/* → relative, @/contexts/session-context →
 * @/features/chat-agent/contexts/session-context, @/hooks/useConversationSettings →
 * @/features/chat-agent/hooks/use-conversation-settings, @/hooks/useGlobalChatListeners →
 * @/features/chat-agent/hooks/use-global-chat-listeners, @/atoms/* →
 * @/features/chat-agent/atoms/*, @/lib/tauri-bridge → @/features/chat-agent/lib/tauri-bridge-stub,
 * @/lib/chat-types → @/features/chat-agent/lib/chat-types, @/lib/utils (cn) → @/shared/lib/cn.
 */
...
import { AgentRecommendBanner } from './agent-recommend-banner'
import { PromptEditorSidebar } from './prompt-editor-sidebar'
import type { InlineEditSubmitPayload } from './chat-message-item'
import {
  conversationsAtom,
  streamingStatesAtom,
...
import type { PendingAttachment, ChatPendingMessage } from '@/features/chat-agent/atoms/chat-atoms'
import { promptConfigAtom, promptSidebarOpenAtom } from '@/features/chat-agent/atoms/system-prompt-atoms'
import { activeToolIdsAtom } from '@/features/chat-agent/atoms/chat-tool-atoms'
...
} from '@/features/chat-agent/hooks/use-conversation-settings'
import { registerPendingTitle } from '@/features/chat-agent/hooks/use-global-chat-listeners'
import { draftSessionIdsAtom } from '@/features/chat-agent/atoms/draft-session-atoms'
import { activeProviderModelAtom } from '@/features/chat-agent/atoms/active-model'
...
import type { ChatMessage, FileAttachment, AttachmentSaveInput } from '@/features/chat-agent/lib/chat-types'
...
function ChatViewInner({ conversationId }: ChatViewProps): React.ReactElement {
  ...
  const [selectedModel, _setSelectedModel] = useConversationModel()
  // eslint-disable-next-line @typescript-eslint/no-unused-vars
  const [_contextLength] = useConversationContextLength()
  const [thinkingEnabled] = useConversationThinkingEnabled()
  // eslint-disable-next-line @typescript-eslint/no-unused-vars
  const [_conversationPromptId] = useConversationPromptId()
  ...
  // promptConfigAtom / userProfileAtom / activeToolIdsAtom are consumed by uclaw's
  // ChatView for context/tool-state logic not yet wired in Hermes (verbatim port).
  void useAtomValue(promptConfigAtom)
  void useAtomValue(userProfileAtom)
  const promptSidebarOpen = useAtomValue(promptSidebarOpenAtom)
  void useAtomValue(activeToolIdsAtom)
  ...
  const syncContextDividers = React.useCallback(async (
    ...
    if (newDividers.length !== currentDividers.length) {
      ...
      await updateContextDividers(convId, newDividers)
    }
    return newDividers
  }, [])
  ...
  const handleSend = React.useCallback(async (
    ...
    if (consumePending) {
      ...
      for (const att of currentAttachments) {
        const base64Data: string | undefined = window.__pendingAttachmentData?.get(att.id!)
      ...
      for (const att of currentAttachments) {
        ...
        window.__pendingAttachmentData?.delete(att.id as string)
      }
  ...
  const truncateFromMessage = React.useCallback(async (
    ...
    const updatedMessages = await truncateMessagesFrom(
      conversationId,
      messageId,
    ...
    if (inlineEditingMessageId && inlineEditingMessageId !== messageId) {
      const stillExists = updatedMessages.some((msg: any) => msg.id === inlineEditingMessageId)
      if (!stillExists) {
        setInlineEditingMessageId(null)
      }
    }
  ...
  const handleStop = React.useCallback((): void => {
    ...
    stopGeneration(conversationId).catch(console.error)
  ...
  const handleDeleteMessage = React.useCallback(async (messageId: string): Promise<void> => {
    try {
      const updatedMessages = await deleteMessageIPC(
        conversationId,
        messageId
      )
      setMessages(updatedMessages)
  ...
  const handleSubmitInlineEdit = React.useCallback(async (
    ...
    try {
      ...
      for (const removed of removedOldAttachments) {
        await deleteAttachment(removed.localPath)
      }
