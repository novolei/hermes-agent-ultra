/**
 * workspace-switcher-bar — ARC-style horizontal bar at the bottom of the
 * left sidebar.
 *
 * Plan 3.2 — ported from uclaw ui/src/components/workspace/WorkspaceSwitcherBar.tsx (589 LOC).
 * Retargets:
 ...
 *   @/views/Kaleidoscope/KaleidoscopeIcon → ./kaleidoscope-icon (co-located)
 *
 * Layout: [workspace icons or dots] | [+]
 *
 * Icons are lucide-react glyphs mapped from the workspace's stored
 * icon value. The compact bar prefers glyphs for visual density; the
 * human-readable name is preserved in the WorkspaceHeader (top of
 * sidebar) and in the hover tooltip.
 *
...
import { KaleidoscopeIcon } from './kaleidoscope-icon'
 ...
 */
const iconForWorkspace = (icon: string | null): LucideIcon => getWorkspaceIcon(icon ?? '')
    ...
    <>
      <TooltipProvider delayDuration={0}>
        <div className="flex items-center gap-1.5 px-3 py-2 border-t border-border/40">
          {/* Kaleidoscope entry — it's not a workspace, so it's separated
              from the workspace dots by a vertical hairline. */}
          <KaleidoscopeIcon
