/**
 * workspace-switcher-bar.test.tsx
 *
 * Plan 3.2 — ported from uclaw ui/src/components/workspace/WorkspaceSwitcherBar.test.tsx (173 LOC).
 * Retargets:
 ...
 *     instead of uclaw's path/sortOrder/createdAt/updatedAt — makeWs updated.
 */
...
vi.mock('@/lib/bridge/workspaces', () => ({
  ...
  createWorkspace: vi.fn().mockResolvedValue({ id: 'new', name: 'new', icon: 'Folder', cwd: null, color: null, position: 0, created_at: null, updated_at: null }),
  updateWorkspace: vi.fn().mockResolvedValue(true),
  deleteWorkspace: vi.fn().mockResolvedValue(true),
}))

function makeWs(id: string, name: string, position = 0, icon: string | null = 'Folder'): WorkspaceInfo {
  return {
    id, name, icon, cwd: `/tmp/${id}`, color: null, position,
...
describe('WorkspaceSwitcherBar', () => {
  ...
  it('renders all icons (full) when workspaces.length ≤ 5', () => {
    ...
    store.set(workspacesAtom, [
      makeWs('w1', 'A', 0, 'Folder'),
      makeWs('w2', 'B', 1, 'Briefcase'),
      makeWs('w3', 'C', 2, 'Rocket'),
    ])
  ...
  it('clicking a workspace icon calls setActiveWorkspaceId', async () => {
    const { setActiveWorkspaceId } = await import('@/lib/bridge/workspaces')
    const store = createStore()
    store.set(workspacesAtom, [
      makeWs('w1', 'A', 0),
      makeWs('w2', 'B', 1),
