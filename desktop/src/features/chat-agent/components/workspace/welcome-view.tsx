// Ported verbatim from uclaw views/WelcomeView.tsx — Plan AS.b
import * as React from 'react'
import { useAtomValue, useSetAtom, useAtom } from 'jotai'
import { ArrowUp, Plus, FolderOpen } from 'lucide-react'
import { cn } from '@/shared/lib/cn'
import { workspacesAtom, activeWorkspaceIdAtom, refreshWorkspacesAtom, type WorkspaceInfo } from '@/features/chat-agent/atoms/workspace'
import { agentSessionsAtom, agentChannelIdAtom, agentModelIdAtom, agentSessionChannelMapAtom, agentSessionModelMapAtom, currentAgentWorkspaceIdAtom } from '@/features/chat-agent/atoms/agent-atoms'
import { activeViewAtom } from '@/features/chat-agent/atoms/active-view'
import { WorkspaceCreateDialog } from '@/features/chat-agent/components/workspace/workspace-create-dialog'
import { useOpenSession } from '@/features/chat-agent/hooks/use-open-session'
import { createAgentSession } from '@/features/chat-agent/lib/tauri-bridge-stub'
import type { AgentSessionMeta } from '@/features/chat-agent/lib/agent-types'
...
export default function WelcomeView(): React.ReactElement {
  ...
  const handleWorkspaceCreated = (ws: WorkspaceInfo): void => {
    refreshWorkspaces()
            ...
            <div className="flex flex-col gap-0.5">
              {recentSessions.map((session) => {
                ...
                return (
                  ...
                  >
                    <span className="text-[15px] leading-none flex-shrink-0" style={{ fontFamily: "'Noto Emoji', sans-serif" }}>{session.titleEmoji || '💬'}</span>
