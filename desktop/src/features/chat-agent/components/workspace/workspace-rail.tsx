 ...
 * workspace-rail.tsx
 *
 * Plan 3.2 — ported from uclaw ui/src/components/workspace/WorkspaceRail.tsx.
 *
 ...
 *   adjusted in agentWorkspaces memo.
 */
...
export function WorkspaceRail({
  ...
  // Desktop WorkspaceInfo uses cwd/position/created_at/updated_at instead of
  // uclaw's path/sortOrder/createdAt/updatedAt. Map accordingly.
  const agentWorkspaces: AgentWorkspace[] = React.useMemo(
    () => workspaces.map((w) => ({
      ...
      icon: w.icon ?? '',
      path: w.cwd ?? null,
      sortOrder: w.position,
      createdAt: w.created_at != null ? w.created_at * 1000 : Date.now(),
      updatedAt: w.updated_at != null ? w.updated_at * 1000 : Date.now(),
    })),
    [workspaces],
  )
