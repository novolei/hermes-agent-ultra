 ...
 * - system 消息独立渲染
 *
 * Ported from uclaw/ui/src/components/agent/SDKMessageRenderer.tsx
 * Retargets applied per Plan 2b.2.c methodology.
 */
...
import { ContentBlock } from '@/features/chat-agent/components/content-block'
import { TaskProgressCard, TASK_TOOL_NAMES } from '@/features/chat-agent/components/task-progress-card'
import { DurationBadge } from '@/features/chat-agent/components/agent-messages'
import {
  Message,
  MessageHeader,
  MessageContent,
...
function extractTurnUsage(turnMessages: SDKMessage[]): { durationMs?: number; usage?: AgentEventUsage; contextWindow?: number } {
...
export function AssistantTurnRenderer({ turn, allMessages, onFork, onRewind, onRetry, onRetryInNewSession, onCompact, isStreaming, stoppedByUser, sessionModelId: _sessionModelId }: AssistantTurnRendererProps): React.ReactElement | null {
  ...
  React.useEffect(() => {
    ...
    readAttachment(file.path)
      .then((base64: string | null) => {
        if (base64) setImageSrc(`data:${mediaType};base64,${base64}`)
      })
      .catch((err: unknown) => console.error('[AttachedImageThumb] 读取附件失败:', err))
  }, [file.path, file.filename])

  const handleSave = React.useCallback((): void => {
    saveImageAs({ localPath: file.path, filename: file.filename, mediaType: 'image/png' })
  }, [file.path, file.filename])
...
function ErrorMessage({ message, onRetry, onRetryInNewSession, onCompact }: ErrorMessageProps): React.ReactElement {
  ...
  const contentText = contentArr
    .filter((b: SDKContentBlock) => b.type === 'text' && 'text' in b)
    .map((b: SDKContentBlock) => (b as { text: string }).text)
    .join('\n') ?? errorText
