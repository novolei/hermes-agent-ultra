...
export function AgentHeartbeatBanner({ sessionId }: AgentHeartbeatBannerProps) {
  ...
  const handleInterrupt = React.useCallback(async () => {
    ...
    try {
      ...
      const payload = (await interruptCurrentAgentRun({
        sessionId,
      })) as {
        partialText: string
        iteration: number
...
/**
 * Stub for consume_pending_recovery Tauri command.
 * Plan 4.x stub — returns null or the recovery payload.
 */
async function consumePendingRecovery(_input: { sessionId: string }): Promise<RecoveryPayload | null> {
  // Stub implementation — returns null in test environment
  return null
}

/**
 * Stub for interrupt_current_agent_run Tauri command.
 * Plan 4.x stub — throws NOT_IMPLEMENTED.
 */
async function interruptCurrentAgentRun(_input: { sessionId: string }): Promise<any> {
  throw new Error('NOT_IMPLEMENTED_IN_PLAN_4_X_BACKEND: interrupt_current_agent_run')
}

/**
 * Stub for dismiss_pending_recovery Tauri command.
 * Plan 4.x stub — throws NOT_IMPLEMENTED.
 */
async function dismissPendingRecovery(): Promise<void> {
  throw new Error('NOT_IMPLEMENTED_IN_PLAN_4_X_BACKEND: dismiss_pending_recovery')
}
