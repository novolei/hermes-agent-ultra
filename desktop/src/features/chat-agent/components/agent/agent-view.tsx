...
} from '@/features/chat-agent/components/composer/composer-mention-controller'
...
import { getActiveAccelerator, getAcceleratorDisplay } from '@/features/chat-agent/lib/shortcut-registry'
import {
...
import type { AgentContextStatus } from '@/features/chat-agent/atoms/agent-atoms'
import {
  agentQueuedMessagesMapAtom,
  removeQueuedMessage,
  type QueuedAgentMessage,
} from '@/features/chat-agent/atoms/agent-queue-messages'
import { activeProviderModelAtom } from '@/features/chat-agent/atoms/active-model'
import { channelsAtom, thinkingExpandedAtom } from '@/features/chat-agent/atoms/chat-atoms'
...
import { sendWithCmdEnterAtom } from '@/features/chat-agent/atoms/shortcut-atoms'
import { agentStatusBarEnabledAtom } from '@/features/chat-agent/atoms/ui-preferences'
import type { AgentSendInput, AgentMessage, AgentPendingFile } from '@/features/chat-agent/lib/agent-types'
import { fileToBase64 } from '@/features/chat-agent/lib/file-utils'
import { createClipboardTextFile } from '@/features/chat-agent/lib/clipboard-attachment'
import {
  updateSettings,
  getAgentSessionPath,
...
interface AgentThinkingPopoverProps {
  agentThinking: import('@/features/chat-agent/lib/agent-types').ThinkingConfig | undefined
  onToggle: () => void
}
...
export function AgentView({ sessionId }: { sessionId: string }): React.ReactElement {
  ...
  return (
    ...
    <AgentSessionProvider sessionId={sessionId}>
      {/* 主内容区域 */}
      <div data-testid="agent-view" className="flex flex-col h-full flex-1 min-w-0 relative">
        {/* Agent Header */}
    ...
    <FirstRunDialog
      ...
      onReady={() => { window.dispatchEvent(new CustomEvent('hermes:stt-start-after-ready')) }}
    />
    </>
  )
}
