 ...
 *
 * Ported from uclaw/ui/src/components/ai-elements/conversation.tsx (Task 28).
 * The `ConversationContextValue` interface and `useConversationContext` hook are
 * intentionally exported (matching the Task 24 forward-stub surface) so that
 * scroll-minimap.tsx continues to type-check without modification.
 */
...
export function Conversation({ resize, className, children }: ConversationProps): React.ReactElement {
  ...
  // potential callers added in future sub-tasks that need in-tree access).
  void computeIsAtBottom
  void userScrollingRef
