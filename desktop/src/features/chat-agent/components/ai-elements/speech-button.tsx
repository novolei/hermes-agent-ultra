 ...
 * window 事件桥接：
 *   - 'hermes:stt-start' → SttModal 调 session.start()
 *   - 'hermes:stt-end'   → SttModal 调 session.end()
 *   - 'hermes:stt-start-after-ready' → 模型下载完后由 FirstRunDialog 派发
 */
...
import {
  ...
  activeComposerAtom,
  type ComposerKind,
} from '@/features/chat-agent/atoms/stt-atoms'
...
export function SpeechButton({
  ...
  const handleClick = React.useCallback(() => {
    ...
    if (isOpenHere) {
      window.dispatchEvent(new CustomEvent('hermes:stt-end'))
      return
    }
    ...
    window.dispatchEvent(new CustomEvent('hermes:stt-start'))
