...
// Re-export MessageAction so callers that import it from this barrel continue
// to compile without changes (convenience re-export only; the canonical
// definition lives in ./message-action).
...
export function MessageHeader({
  ...
  /** Display name of the agent (e.g. "Hermes"). Configurable per-session.
   *  When omitted, falls back to "Assistant". */
  name?: string
  /** Underlying model id (e.g. "claude-sonnet-4-5") — rendered as
   *  caption next to the agent name. Optional. */
  model?: string
  time?: string
  logo?: React.ReactNode
}): React.ReactElement {
  const primaryName = name ?? 'Assistant'
...
/** 链接渲染器：外部 URL 用 Tauri openUrl 打开 */
    ...
    <a
      ...
      onClick={(e) => {
        if (href && (href.startsWith('http://') || href.startsWith('https://'))) {
          e.preventDefault()
          // Open external links via Tauri opener plugin (avoids in-webview navigation).
...
// Plan 3.5 note: when real chips land, FileChipFromHast will call
// useFileChipResolver with IPC batching. For now the stub resolver
// always returns a pending entry and FilePathChip renders plain code.
...
export function MessageLoading({ startedAt: _startedAt }: { startedAt?: number }): React.ReactElement {
  return (
