// Ported verbatim from uclaw components/preview/renderers/DocxRenderer.tsx — Plan PV.b
...
export function DocxRenderer({ bytes, name }: DocxRendererProps): React.ReactElement {
  ...
  React.useEffect(() => {
    ...
    void (async () => {
      try {
        const { convertDocxToHtml } = await import('@/features/chat-agent/components/preview/office-parsers/docx')
