// Ported verbatim from uclaw components/preview/renderers/PptxRenderer.tsx — Plan PV.b
...
export function PptxRenderer({ bytes, name }: PptxRendererProps): React.ReactElement {
  ...
  React.useEffect(() => {
    ...
    void (async () => {
      try {
        const { convertPptxToHtml } = await import('@/features/chat-agent/components/preview/office-parsers/pptx')
