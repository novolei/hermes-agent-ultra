// Ported verbatim from uclaw components/preview/renderers/XlsxRenderer.tsx — Plan PV.b
...
export function XlsxRenderer({ bytes, name }: XlsxRendererProps): React.ReactElement {
  ...
  React.useEffect(() => {
    ...
    void (async () => {
      try {
        const { convertXlsxToHtml } = await import('@/features/chat-agent/components/preview/office-parsers/xlsx')
