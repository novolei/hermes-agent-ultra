...
import { sttModalStateAtom, type ComposerKind } from '@/features/chat-agent/atoms/stt-atoms'
import { useSttStreamingSession } from '@/features/chat-agent/hooks/use-stt-streaming-session'
import './stt-modal.css'
...
export function SttModal({ composer, onSegmentFinalized }: SttModalProps): React.ReactElement | null {
  ...
  React.useEffect(() => {
    ...
    window.addEventListener('hermes:stt-start', onStart)
    window.addEventListener('hermes:stt-end', onEnd)
    window.addEventListener('hermes:stt-start-after-ready', onStart)
    return () => {
      window.removeEventListener('hermes:stt-start', onStart)
      window.removeEventListener('hermes:stt-end', onEnd)
      window.removeEventListener('hermes:stt-start-after-ready', onStart)
    }
  }, [state.kind, session])
