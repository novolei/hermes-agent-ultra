/**
 * chat-tool-block.tsx — Chat 模式工具调用块（紧凑列表行样式）
 *
 * Ported from uclaw/ui/src/components/chat/ChatToolBlock.tsx.
 * Renamed to kebab-case; exported symbol stays `ChatToolBlock`.
 *
 *   🔧 bash ls -a
 *   🔧 bash rm -rf …
 *   ⟳  bash long-running …
...
import { getToolIcon } from '@/features/chat-agent/lib/tool-utils'
import { getToolPhrase } from '@/features/chat-agent/lib/tool-phrase'
import { ToolResultRenderer } from '@/features/chat-agent/components/tool-renderers'
import { BashStreamView } from '@/features/chat-agent/components/tool-renderers/bash-stream-view'
import type { LiveOutput } from '@/features/chat-agent/atoms/agent-atoms'
